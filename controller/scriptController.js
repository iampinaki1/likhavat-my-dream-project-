...
// export const loadScriptsAll = async (req, res) => {
//   try {
...
//   try {
//     const scriptId = req.param.Script;
//     const username = req.userId;
//     // const script = await Script.find(_id); why not this?
//     const script = await Content.findById(scriptId);
//     if (!script) {
//       return res.status(404).json({ msg: "not found" });
//     }
//     if (script.visibility === "restricted") {
//       return res.status(404).json({ msg: "not found" });
//     }
//     script.allowedUsers.findOne(username);
//     return res.status(200).json(script);
//   } catch (err) {
//     console.error(err);
//     return res.status(500).json({ error: "server error" }); //agar return na likhu
//   }
// };
...
//     let sortOrder = { _id: -1 };
//     if (sort) {
//       if (sort === "asc") sortOrder = { likes: 1 };
//       else if (sort === "desc") sortOrder = { likes: -1 };
//       else if (sort === "oldest") sortOrder = { _id: 1 };
//     }
//     //allow only visible
//     const filter = "public";
//     const script = await Script.find({ visibility: filter }).sort(sortOrder);
//     return res.status(200).json(script);
//   } catch (err) {
//     console.error(err);
//     return res.status(500).json({ error: "server error" }); //agar return na likhu
//   }
// };
// export const likedScript = async (req, res) => {
//   try {
//     const userId = req.userId;
//     const scriptId = req.params.id;
//     const script = await Content.findById(scriptId);
//     if (!script) return res.status(404).json({ message: "Post not found" });
//     // Check if user already liked
//     if (post.likes.includes(userId))
//       return res.status(400).json({ message: "Already liked" });
//     script.likes.push(userId);
//     await script.save();
//     res.status(200).json({ message: "Liked", totalLikes: post.likes.length });
//   } catch (err) {
//     console.error(err);
//     return res.status(500).json({ error: "server error" }); //agar return na likhu
//   }
...
// export const unlikedScript = async (req, res) => {
//   try {
//     const userId = req.userId;
//     const scriptId = req.params.id;
//     const script = await Content.findById(scriptId);
//     if (!script) return res.status(404).json({ message: "Post not found" });
//     script.likes.pull(userId);
//     await script.save();
//     res.status(200).json({ message: "Unliked", totalLikes: post.likes.length });
//   } catch (err) {
//     console.error(err);
//     return res.status(500).json({ error: "server error" }); //agar return na likhu
//   }
...
//delete
//edit //i mean add new version
//comment
//create (set collab and visibility and other parameters)
...
//     const script = await Script.findById( {_id:id} );

//     if (!script) {
//       return res.status(404).json({ message: "Content not found" });
//     }
// export const loadScript = async (req, res) => {
//   try {
//     const { lastId } = req.query; //for post

//     const query = lastId ? { _id: { $lt: lastId } } : {};
//     const scripts = await Script.find(query, { visibility: "public" })
//       .sort({ _id: -1 }) // newest first
...
//       success: true,
//       scripts,
//       nextCursor: posts.length > 0 ? scripts[scripts.length - 1]._id : null,
//     });
//   } catch (err) {
//     res.status(500).json({ error: err.message });
//   }
